<table class="table mt-2 border-primary">   
    <tr class=" bg-primary"> 
        <th>DATE</th> 
        <th>DESIGNATION</th>  
        <th>TYPE</th>  
...
<?php foreach ($Data as $value){?>    
    <tr id="search-result">     
        ...
        <a onclick="DeleteAchat(<?php echo $id=$value['id_Acchat']?>)" > 
            <svg class="ms-2 cursor-pointer " xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash-fill" viewBox="0 0 16 16" style="cursor:pointer">
        ...
        <a onclick="UpdateAchat(<?php echo $value['id_Acchat']?>,<?php echo  $value['date']?>,'<?php echo $value['Designation'] ?>','<?php echo $value['Type'] ?>','<?php echo $value['Fas']?>','<?php echo $value['Quantité'] ?>','<?php echo $value['prix_Unitaire']?>','<?php echo $value['Mantant_HT']?>','<?php echo $value['Chantier']?>')">
            <svg class="ms-3" xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16"  data-bs-toggle="modal" data-bs-target="#exampleModal" style="cursor:pointer">
